...
class NASBench201API(NASBenchMetaAPI):
  ...
  def query_test_acc_by_index(self, index, dataset):
    assert dataset in ['cifar10', 'cifar100', 'ImageNet16-120'], 'Invalid dataset : {:}.'.format(dataset)
    result = self.arch2infos_dict[index]['200']
    if dataset == 'cifar10':
      test_acc = result.get_metrics(dataset, 'ori-test')['accuracy']
    else:
      test_acc = result.get_metrics(dataset, 'x-test')['accuracy']
    return test_acc
