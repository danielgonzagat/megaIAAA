import torch
from eval_architecture.ofa.utils.layers import *
from eval_architecture.ofa.utils.layers import MBConvLayer as MBInvertedConvLayer
from eval_architecture.ofa.utils.my_modules import *
from eval_architecture.ofa.networks.mobilenet_v3 import MobileNetV3
...
class MobileInvertedResidualBlock(MyModule):
    ...
    def forward(self, x):
        if self.mobile_inverted_conv is None or isinstance(
            ...
            if self.drop_connect_rate > 0.0:
                res = drop_path(
                    res, drop_prob=self.drop_connect_rate, training=self.training
                )
    ...
    def build_from_config(config):
        mobile_inverted_conv = set_layer_from_config(config["conv"])
...
class NSGANetV2(MobileNetV3):
    ...
    def build_from_config(config, drop_connect_rate=0.0):
        ...
        for block_idx, block_config in enumerate(config["blocks"]):
            block_config["drop_connect_rate"] = (
                drop_connect_rate * block_idx / len(config["blocks"])
            )
