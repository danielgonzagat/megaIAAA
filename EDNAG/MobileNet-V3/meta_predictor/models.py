...
sys.path.append("./")
from meta_predictor.set_encoder.setenc_models import SetPool


class PredictorModel(nn.Module):
    def __init__(self, hs, nz, num_sample, graph_config):
        ...
        # 2. gate-related
        self.gate_forward = nn.Sequential(nn.Linear(self.vs, self.hs), nn.Sigmoid())
        self.gate_backward = nn.Sequential(nn.Linear(self.vs, self.hs), nn.Sigmoid())
        ...
        self.pred_fc = nn.Sequential(
            nn.Linear(input_dim, self.hs), nn.Tanh(), nn.Linear(self.hs, 1)
        )
    ...
    def _one_hot(self, idx, length):
        if type(idx) in [list, range]:
            if idx == []:
                return None
            # 如果 idx 的元素是 tensor 并且每个元素只有一个值，转换为 int 列表
            if type(idx == list):
                if isinstance(idx[0], torch.Tensor) and idx[0].numel() == 1:
                    idx = [int(x.item()) for x in idx]
    ...
    def _propagate_to(
        self, G, v, propagator, H=None, reverse=False, gate=None, mapper=None
    ):
    ...
    def set_encode(self, X):
        proto_batch = []
        for x in X:
            cls_protos = self.intra_setpool(x.view(-1, self.num_sample, 512)).squeeze(1)
