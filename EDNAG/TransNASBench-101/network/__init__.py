...
def get_cell_based_tiny_net(config):
  ...
  group_names = ['DARTS-V1', 'DARTS-V2', 'PCDARTS', 'GDAS', 'SETN', 'ENAS', 'RANDOM', 'RANK', 'ONE-SHOT-SUPERNET']
  if super_type == 'basic' and config.name in group_names:
    from .cell_searchs import nas201_super_nets as nas_super_nets
    try:
      if config.name != 'PCDARTS':
        ...
        return nas_super_nets[config.name](config.C, config.N, config.max_nodes, config.num_classes, config.space,
                                           config.affine, config.track_running_stats, config.pcdarts_prune_ratio)
    except:
      return nas_super_nets[config.name](config.C, config.N, config.max_nodes, config.num_classes, config.space)
