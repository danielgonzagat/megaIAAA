...
class ModelDB(object):
    ...
    def save(self, model_dic, epoch, metrics):
        ...
        assert len(metrics.keys()) == len(
            self.cfg["metric_content"]
        ), f"Received metrics {metrics.keys()}"
    ...
    def create_initial_dic(self):
        initial_version_dic = {
            ...
            "inference_time": get_inference_time(
                self.cfg["model_type"], (1, 3, 720, 1080)
            ),
            "metrics": {},
        }
        if "discriminator" in self.cfg.keys():
            initial_version_dic["discriminator_ckpt_path"] = (
                self.discriminator_ckpt_path
            )
