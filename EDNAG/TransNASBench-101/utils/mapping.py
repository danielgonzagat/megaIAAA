...
class Exp(Identity):
    ...
    def forward(self, x):
        ...
        return torch.exp(x / self.temperature) / torch.exp(torch.tensor(self.max_fitness_val) / self.temperature)
    ...
    def forward(self, x: torch.Tensor, task = None):
        if task is None:
            ...
            for i in range(x.shape[0]):
                ...
                if x[i] <= 50.01 or x[i] >= 99999:
                    y[i] = 0.0
        return y

    def __call__(self, x, task=None):
